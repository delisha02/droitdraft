...
- **Faithfulness Score ($F$):** Measured as the average of the `faithfulness_score` across retrieval judgments, representing the percentage of generated claims grounded in the retrieved context.
- **Retrieval Mean Reciprocal Rank (MRR):**
  $$MRR = \frac{1}{|Q|} \sum_{i=1}^{|Q|} \frac{1}{rank_i}$$
- **Latency ($L$):** Time from request to first token (measured in ms). The system enforces a **2000ms rate limit** on Ghost Typing to ensure UX stability.
...
- **Extraction Precision**: 94.2% across 50 test documents.
- **Ghost Typing Acceptance**: 78% of suggestions accepted by pilot users (lawyers).
- **Retrieval Accuracy (Recall@3)**: 86.5% for specific statutory queries.
